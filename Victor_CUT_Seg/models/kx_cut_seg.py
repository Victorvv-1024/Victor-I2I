...
class KX_CUT(BaseModel):
    ...
    def backward_S(self):
        ...
        loss_S_real = self.criterionSEG(fake_mask_A, self.mask_A).mean()

        # fake_B = self.fake_B.detach()
        # loss_S_fake = self.criterionSEG(self.netS(fake_B), self.mask_A).mean()

        self.loss_S = loss_S_real
